...
class TestStateResetBug(TestCase):
    ...
    def test_partial_field_sync_preserves_other_fields(self):
        ...
        if 'start_date' in result['state']:
            self.assertEqual(result['state']['start_date'], '2026-01-15',
    ...
    def test_multiple_field_changes_preserve_all_previous(self):
        ...
        result = component.process_action(
            '_sync_field',
            {'field': 'start_date', 'value': '2026-01-15'},
            state
        )
        if result['partial']:
            for key, value in result['state'].items():
        ...
        result = component.process_action(
            '_sync_field',
            {'field': 'monthly_rent', 'value': 1500.0},
            state
        )
        if result['partial']:
            for key, value in result['state'].items():
    ...
    def test_nested_field_sync_preserves_sibling_fields(self):
        ...
        result = component.process_action(
            '_sync_field',
            {'field': 'create_buffer.property_id', 'value': '123'},
            state
        )
        ...
        self.assertEqual(result['state']['create_buffer']['property_id'], '123',
                        "property_id should be updated")

        # CRITICAL: start_date should be preserved in the returned create_buffer
        self.assertEqual(result['state']['create_buffer']['start_date'], '2026-01-15',
    ...
    def test_sync_field_returns_minimal_state(self):
        ...
        result = component.process_action(
            '_sync_field',
            {'field': 'monthly_rent', 'value': 2500.0},
            state
        )
        ...
        if len(result['state']) == 1:
            self.assertEqual(list(result['state'].keys()), ['monthly_rent'],
...
class TestRealWorldLeaseForm(TestCase):
    ...
    def test_lease_form_complete_user_flow(self):
        ...
        self.assertEqual(browser_state['start_date'], '2026-01-15',
                        "BUG FIX: start_date should be preserved after property selection")
        self.assertEqual(browser_state['property_id'], '123',
                        "property_id should be set")
        ...
        self.assertEqual(browser_state['property_id'], '123',
                        "property_id should still be there")
        self.assertEqual(browser_state['tenant_id'], '456',
        ...
        self.assertEqual(browser_state['property_id'], '123',
                        "property_id preserved through entire flow")
        self.assertEqual(browser_state['tenant_id'], '456',
                        "tenant_id preserved through entire flow")
        self.assertEqual(browser_state['start_date'], '2026-01-15',
                        "start_date preserved through entire flow - BUG FIXED!")
        self.assertEqual(browser_state['monthly_rent'], 1500.0,
                        "monthly_rent should be set")
